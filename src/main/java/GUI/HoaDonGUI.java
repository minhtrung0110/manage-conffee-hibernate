...
//        btnBack = new JLabel(new ImageIcon("./src/image/btnBack.png"));
        ...
        btnDelete.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                ...
                if (i == 0) {
                    orderBLL.delete(txtMaHD.getText());
                    orderDetailBLL.delete(txtMaHD.getText());
                    JOptionPane.showMessageDialog(null,"Xóa hóa đơn thành công !!!");
                    cleanView();
                    tbl.clearSelection();
                    outModel(model, (ArrayList<Order>) orderBLL.getOrderBLL());
                }
            }
        });
...
//        /******************************************/
    ...
    public void outModel(DefaultTableModel model, ArrayList<Order> hd) {     //xuat tu arraylist len table
        ...
        for (Order h : hd) {
            data = new Vector();
            data.add(h.getId());
            data.add(h.getCustomer().getId());
            data.add(h.getId_Staff());
            data.add(h.getCreatedDate());
            data.add(h.getTotalPrice());
            model.addRow(data);
        }
        tbl.setModel(model);
    }
